...
export class AuthService {
  ...
  private async authenticateUser(
    ...
    if (user && user.password !== null) {
      ...
      await this.setupService.setupPersonalSpace(user, workspaceId);
      return user;
    }
    ...
    if (user && user.password === null) {
      ...
      const parsedRoles = this.setupService.parseKeycloakRole(keycloakUser.roles);

      if (parsedRoles.length > 0) {
        await this.setupService.setupUserSpaceMemberships(user, parsedRoles, workspaceId);
        await this.setupService.syncUserSpaceRoles(user.id, workspaceId, parsedRoles);
      }
    ...
    if (!authenticatedUser) {
      ...
      if (parsedRoles.length > 0) {
        await this.setupService.setupUserSpaceMemberships(authenticatedUser, parsedRoles, workspaceId);
        await this.setupService.syncUserSpaceRoles(authenticatedUser.id, workspaceId, parsedRoles);
      }
    }

    await this.setupService.setupPersonalSpace(authenticatedUser, workspaceId);
